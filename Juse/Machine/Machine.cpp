...
template ByteSet Juse::word2set<A24>(A24);
...
void Machine::createPool(PoolId pool_index) {
    memory->insert({ pool_index, makeS<Pool>() });
}

void Machine::createSegment(PoolId pool_index, SegmentId segment_index) {
...
SPtr<Pool> Juse::Machine::getPool(PoolId pool_index) {
    return Utility::MachineMemory::pool(memory, pool_index);
}

SPtr<Segment> Juse::Machine::getSegment(PoolId pool_index, SegmentId segment_index) {
...
ByteSet Machine::readData(Cpu& cpu, DataId address, size_t nb_bytes) {
    return readAt(Address::with(cpu.data_addr.pool, cpu.data_addr.segment, address), nb_bytes);
}
...
void Juse::Machine::writeData(Cpu& cpu, DataId address, ByteSet set) {
    writeAt(Address::with(cpu.data_addr.pool, cpu.data_addr.segment, address), set);
}
