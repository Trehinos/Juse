...
void Cpu::cycle(Machine &machine, bool debug) {
  ...
  if (debug) {
    machine.out << "@" << std::hex << std::right << std::setfill('0')
                << std::setw(4);
    machine.out << (int)instruction_pointer << " : ";
  }
  Instruction instruction = {ByteSet{U8((MASK_16TOP8 & identifier) >> 8),
                                     U8(MASK_BOTTOM8 & identifier)}};
  ...
  if (debug) {
    ...
    for (Argument argument : current->getArgumentDefs()) {
      machine.out << std::left << std::setfill('0') << std::setw(argument.size)
                  << std::right << std::hex
                  << current->argument(instruction, index++);
    }
...
U8 Cpu::data(Memory &memory) { return dataAt(memory, addressPointer()); }
